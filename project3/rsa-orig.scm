...
(define (solve-ax+by=1 a b)
  (define (solve-iter c d)
    (if (= c 1)(cons 1 0)
        (let ((r (remainder c d))
              (q (truncate (/ d c)))
              (x (car (solve-iter (modulo d c) c )))
              (y (cdr (solve-iter (modulo d c) c ))))
          (cons (- y (* q x)) x))))
  (if (> b a)
      (solve-iter b a)
      (solve-iter a b)))
...
(define (encrypt-and-sign txt pri-key pub-key)
  (let((mes (RSA-encrypt txt pub-key))
       (sig (RSA-transform (compress(RSA-encrypt txt pub-key)) pri-key)))
    (signed-message mes sig)))

;;;Anyone who receives a message can authenticate the
;;;signature by transforming it using your public key and checking that this gives
;;;the same result as applying the compression function to the message.

(define (authenticate-and-decrypt sig-mes pub-key pri-key)
  (if(equal? (RSA-transform (cdr sig-mes) pub-key)
             (compress (message sig-mes)))
     (RSA-decrypt (message sig-mes) pri-key)
...
;;; take n and m and get p using smallest divisor
;;; q = n/p
;;; NOW since we have p and q we can get m
;;; take m and use invert modulo to get de
;;; then we can divide by e to get d
;;; here we have private key n and d
